...
def process_run(csv_path):
    """
    Load a per-run CSV and return:
    - df: the raw DataFrame
    - summary: dict with aggregated stats (one row per run)
    """
    df = pd.read_csv(csv_path, parse_dates=["timestamp"])

    # Ensure required columns exist
    distance = df["distance"].max() if "distance" in df else None
    duration = (df["timestamp"].max() - df["timestamp"].min()).total_seconds() / 60 if "timestamp" in df else None
    avg_pace = None
    ...
    summary = {
        ...
        "total_elev_gain": df["altitude"].diff().clip(lower=0).sum() if "altitude" in df else None,
    }
